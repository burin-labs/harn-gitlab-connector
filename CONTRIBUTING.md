...
`harn-gitlab-connector` is a first-party Burin Labs package. It implements Harn Connector Contract v1 for GitLab: it verifies inbound webhook tokens, normalizes GitLab payloads to the tagged `NormalizeResult` envelope, and dispatches outbound REST and GraphQL calls.
...
No release tag is published yet, so there is no stable surface to contribute against.

## Where to send a GitLab request or bug instead
...
Label it `area/connectors` and say which GitLab events or operations you need
